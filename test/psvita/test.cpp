...
int main(int argc, char* argv[]) {
    ...
    for (uint32_t i = 0; i < render_result.image_count; i++) {
        ...
        char filename[32] = {0};
        snprintf(filename, sizeof(filename), "ux0:data/capi_test_image_%u.png", i);
        png_writer_write_image_c(filename, image);
        // imageTexture = vita2d_load_BMP_buffer(image->bitmap);
        // vita2d_start_drawing();
		// vita2d_clear_screen();

        // vita2d_draw_texture(imageTexture, 0, 0);
